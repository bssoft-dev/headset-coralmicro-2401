...
#include "third_party/nxp/rt1176-sdk/devices/MIMXRT1176/drivers/fsl_sai.h"
#include "complex"
...
//-- add custom code 1
constexpr int kTestSampleRate = 16000; // 샘플레이트는 16000과 48000 지원
constexpr int kTestDurationMs = 975;
constexpr int kTestAudioSize = kTestSampleRate * kTestDurationMs / 1000;
constexpr int kNoiseThreshold = kTestAudioSize * 3000;
//-- end custom code 1
  ...
  if (sample_format == kS32LE) {
    ...
    while (true) {
      ...
      for (size_t i = 0; i < size; ++i) {
        buffer16[i] = buffer32[i] >> 16;
        //-- add custom code 2
        total_value += buffer16[i] * buffer16[i];
        //-- end custom code 2
      }
      if (WriteArray(client_socket, buffer16.data(), size) != IOStatus::kOk)
        break;
      //-- add custom code 3
      if (total_value > kNoiseThreshold){
...
void TestSAI() {  
  // Initialize SAI I2S mode
  // sai_config_t config;
  SAI_Init(SAI2);
  // insert sine wave to test_data
  uint32_t test_data[kTestAudioSize];
  for (int i = 0; i < kTestAudioSize; i++) {
    // test_data[i] = 0x7FFF * sin(2 * 3.141592 * i / kTestSampleRate);
    test_data[i] = 0x7FFF * sin(2 * 3.141592);
  }
  // Write
  for (int i = 0; i < 1000000000000; i++) {
    SAI_WriteData(SAI1, 1, test_data[i % kTestAudioSize]);
  }
  // for (int i = 0; i < kTestAudioSize; i++) {
  //   SAI_WriteData(SAI1, 1, test_data[i]);
  // }
}
